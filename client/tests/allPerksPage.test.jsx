...
describe('AllPerks page (Directory)', () => {
  ...
  test('lists public perks and responds to merchant filtering', async () => {
    ...
    // Determine expected count from the live API to avoid brittle assumptions.
    const { data } = await global.__TEST_CONTEXT__.api.get('/perks/all', {
      params: { merchant: seededPerk.merchant }
    });
    const expectedCount = data.perks.length;
    const summaryRegex = new RegExp(`^Showing ${expectedCount} perk${expectedCount !== 1 ? 's' : ''}$`, 'i');
    ...
    // Wait for the summary to reflect the filtered count.
    await waitFor(() => {
      expect(screen.getByText(summaryRegex)).toBeInTheDocument();
    });

    // The seeded record should be visible among the filtered results.
    expect(screen.getByText(seededPerk.title)).toBeInTheDocument();
  });
});
